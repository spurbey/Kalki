...
- Zero-Dependency State Extraction: For modern SSR pages (such as Inertia.js `<div id="app" data-page="...">` or Next.js `<script id="__NEXT_DATA__">`), extract embedded JSON with standard library `re`, `html.unescape`, and `json.loads(..., strict=False)`. Do NOT install external HTML parsers (`beautifulsoup4`, `lxml`).
- Avoid Search Indices for Entity Details: Do NOT reverse-engineer background search APIs (e.g. Algolia `algolia.net/1/indexes`) when collecting detailed entity records (e.g. founders). Search indices only contain high-level directory cards; extract detailed entity records directly from the reviewed page models.
...
5. Explore the unfamiliar source with the compact `browser_research_*` tools. Inspect one representative page, determine the repeatable URL pattern, and save only the returned facts under `research/`. To persist physical evidence samples for operator authoring and testing (which strictly enforces `--limit 5`), run `PYTHONPATH="$PWD/.kalki/deps:/opt/tf/mcp-client" python -m kalki_runtime.research_cli capture --url <url> --out research/captures/<name>.html` for enough representative items (e.g. 4-5 entities/companies) so that the operator produces at least 5 records during `pipeline_cli test --limit 5`. The coordinator must not call `browser_fetch_pages`; that tool is for generated operators inside the sandbox.
