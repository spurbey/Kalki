...
const jsonHash = (value: unknown) =>
  createHash("sha256").update(canonicalHashJson(value)).digest("hex");

describe("workbook persistence", () => {
  it("uses the shared cross-language JSON hash representation", () => {
    const fixture = JSON.parse(
      readFileSync(
        new URL("../../../../fixtures/hash-contract.json", import.meta.url),
        "utf8",
      ),
  ...
  it("keeps a workbook and task after reopening SQLite", () => {
    ...
    try {
      ...
      const snapshot = new WorkbookService(reopenedDatabase).getSnapshot(
        workbook.id,
      );
  ...
  it("recovers the current TrueForge turn after reopening SQLite", () => {
    ...
    try {
      ...
      const workbook = firstService.createWorkbook({ title: "Tesla research" });
      expect(() =>
        firstService.connectTrueForgeSession(workbook.id, "x".repeat(129)),
      ).toThrow();
      expect(
        firstService.getWorkbook(workbook.id).trueforge_session_id,
      ).toBeNull();
      ...
      expect(
        firstService.getWorkbook(workbook.id).current_trueforge_turn_id,
      ).toBeNull();
      firstService.saveTrueForgeTurn(workbook.id, {
        id: "turn-1",
        sessionId: "session-1",
      ...
      const recoveredTurn = reopenedService.getCurrentTrueForgeTurn(
        workbook.id,
      );
      reopenedDatabase.close();
  ...
  it("stores each streamed TrueForge event sequence once", () => {
    ...
    try {
      ...
      expect(
        service.getCurrentTrueForgeTurn(workbook.id)?.last_sequence_number,
      ).toBe(2);
    } finally {
      database.close();
    }
  ...
  it("validates review decisions and completes clarifications", () => {
    ...
    try {
      ...
      database
        .prepare(
          "UPDATE tasks SET state = 'awaiting_task_confirmation' WHERE id = ?",
        )
        .run(task.id);
      service.connectTrueForgeSession(workbook.id, "session-1");
  ...
  it("persists reviewed schemas and a test run", () => {
    ...
    try {
      ...
      firstDatabase
        .prepare("UPDATE tasks SET state = 'building' WHERE id = ?")
        .run(task.id);
      const run = {
          ...
          parents: [
            {
              ...
              dedupe_key: sourceSamples[index]!.dedupe_key,
            },
          ],
        },
      }));
      ...
      firstService.markQuestionSubmitting(
        workbook.id,
        "tool-production-review",
        productionAnswer,
      );
      expect(
        firstService.getProductionAuthorization({
      ...
      expect(firstService.publishBatch(sourceBatch)).toMatchObject({
        inserted: 5,
        replayed: false,
      });
      expect(firstService.publishBatch(sourceBatch)).toMatchObject({
      ...
      expect(firstService.publishBatch(derivedBatch)).toMatchObject({
        inserted: 3,
        published_row_count: 8,
      });
      ...
      const firstPage = firstService.getTableRows(
        historyTable!.id,
      ...
      const otherTaskHash = createHash("sha256")
        .update(otherTaskMarkdown)
        .digest("hex");
      firstService.registerTask({
      ...
      firstDatabase
        .prepare("UPDATE tasks SET state = 'building' WHERE id = ?")
        .run(otherTask.id);
      firstService.startRun({
        ...run,
        run_id: "run_other_test",
        task_id: otherTask.id,
        task_hash: otherTaskHash,
      });
      firstDatabase.close();
      ...
      expect(context.tables.map((table) => table.slug)).toEqual([
        "tesla-history",
        "tesla-top-3",
      ]);
      ...
      expect(
        context.runs.find((candidate) => candidate.mode === "test")?.status,
      ).toBe("completed");
      expect(
        context.runs.find((candidate) => candidate.mode === "production")
          ?.status,
      ).toBe("completed");
      expect(snapshot.runs[0]?.test_samples?.["tesla-history"]).toHaveLength(5);
      expect(
        context.runs.find((candidate) => candidate.mode === "test")?.counts
          .formal_rows,
      ).toBe(0);
      expect(
        context.runs.find((candidate) => candidate.mode === "production")
      ...
      expect(
        Object.values(snapshot.table_counts).reduce(
          (sum, count) => sum + count,
          0,
        ),
      ).toBe(8);
      expect(
        reopenedDatabase
          .prepare("SELECT count(*) AS count FROM approval_events")
          .get(),
      ).toEqual({ count: 1 });
      reopenedDatabase.close();
